...
if (isset($_GET['id_agendamento'])) {
    ...
    // Consulta para obter os dados do agendamento, paciente e procedimento de glicemia
    $stmt = $pdo->prepare('
        SELECT 
            AG.ID_AGENDAMENTO,
            AG.data_agendamento,
        ...
        WHERE 
            AG.ID_AGENDAMENTO = ? AND PROC.nome_proced = "Glicemia em Jejum"
    ');
    ...
    // Consulta para buscar o último valor cadastrado de glicose
    $stmt = $pdo->prepare('
        SELECT glicemia.glicose 
        FROM glicemia
        INNER JOIN EXAMES_E_AMOSTRAS E ON glicemia.ID_EXAME = E.ID_EXAME
        WHERE E.ID_PROCEDIMENTO = ? AND E.ID_PACIENTE = ?
        ORDER BY glicemia.id_resultado DESC
        LIMIT 1
    ');
    ...
    $valor_glicose = $exame['glicose'] ?? 'N/A'; // Caso não haja valor cadastrado

    // Consulta para obter os valores de referência da glicose
    $stmt = $pdo->prepare('
SELECT 
    valor_min, 
    valor_max, 
...
WHERE 
    nome_parametro = "Glicose"
');
    $stmt->execute();
    $referencias_glicemia = $stmt->fetchAll();
} else {
...
<head>
    <meta charset="UTF-8">
    <title>Aprovação de Exame - Glicemia em Jejum</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>Aprovação do Exame - Glicemia em Jejum</h1>
        ...
        <div class="result-box">
            <h2>Resultado do Exame Glicemia</h2>
            <span><?= htmlspecialchars($valor_glicose) ?> mg/dL</span>
        </div>
        <div class="result-box">
            <h3>Valores de Referência para Glicose</h3>
            <ul style="list-style: none; padding: 0;">
                <?php foreach ($referencias_glicemia as $referencia): ?>
